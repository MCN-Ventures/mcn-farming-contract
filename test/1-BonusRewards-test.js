...
describe("BonusRewards", () => {
  ...
  before(async () => {
    ...
    await lpToken
      ...
      .approve(bonusRewards.address, ETHER_UINT_100000000);
    await lpToken
      .connect(userAAccount)
      .approve(bonusRewards.address, ETHER_UINT_10000);
    await lpToken
      .connect(userBAccount)
      .approve(bonusRewards.address, ETHER_UINT_20000);
  ...
  it("Should only set responders by owner", async function () {
    await expectRevert(
      bonusRewards.connect(partnerAccount).setResponders([partnerAddress]),
  ...
  it("Should addPoolsAndAllowBonus by owner under right condition", async function () {
    ...
    await bonusRewards.addPoolsAndAllowBonus(
      [lpToken.address],
      [bonusToken.address],
      [partnerAddress]
    );
    ...
    const authorizers = await bonusRewards.getAuthorizers(
      lpToken.address,
      bonusToken.address
    );
  ...
  it("Should NOT collectDust on lpToken", async function () {
    await expectRevert(
      bonusRewards.collectDust(lpToken.address, lpToken.address, 0),
      "BonusRewards: lpToken, not allowed"
    );
  });
  ...
  it("Should addBonus by partner correctly", async function () {
    ...
    const [[[bonusTokenAddr, start, end, weeklyRewards, accRewardsPerToken]]] =
      await bonusRewards.getPool(lpToken.address);
    ...
    await expectRevert(
      ...
      "BonusRewards: pool does not exist"
    );
    await expectRevert(
      bonusRewards
        .connect(partnerAccount)
  ...
  it("Should deposit lpToken for userA and userB", async function () {
    ...
    await bonusRewards
      .connect(userBAccount)
      .deposit(lpToken.address, ETHER_UINT_1);
    expect(await lpToken.balanceOf(userBAddress)).to.equal(
      ETHER_UINT_20000.sub(ETHER_UINT_1)
    );
    expect(await lpToken.balanceOf(bonusRewards.address)).to.equal(
      ETHER_UINT_1
    );
    ...
    await bonusRewards
      .connect(userAAccount)
      .deposit(lpToken.address, ETHER_UINT_800);
    expect(await lpToken.balanceOf(userAAddress)).to.equal(
      ETHER_UINT_10000.sub(ETHER_UINT_800)
    );
    expect(await lpToken.balanceOf(bonusRewards.address)).to.equal(
      ETHER_UINT_800.add(ETHER_UINT_1)
    );
  });
  ...
  it("Should extendBonus by partner correctly", async function () {
    ...
    await bonusRewards
      .connect(partnerAccount)
      .extendBonus(lpToken.address, 0, bonusToken.address, WEEKLY_REWARDS);
  ...
  it("Should view minedRewards, claimRewards for userA, deposit and claim for userB", async function () {
    ...
    const [userARewards] = await bonusRewards.viewRewards(
      lpTokenAddress,
      userAAddress
    );
    ...
    await bonusRewards
      .connect(userAAccount)
      .claimRewardsForPools([lpToken.address, lpToken.address]);
    ...
    await bonusRewards
      .connect(userBAccount)
      .deposit(lpTokenAddress, ETHER_UINT_800);
    expect(await lpToken.balanceOf(userBAddress)).to.equal(
      userBInitBal.sub(ETHER_UINT_800)
    );
    expect(await lpToken.balanceOf(bonusRewards.address)).to.equal(
      bonusRewardsInitBal.add(ETHER_UINT_800)
    );
  ...
  it("Should withdraw for userB", async function () {
    ...
    await bonusRewards
      .connect(userBAccount)
      .withdraw(lpTokenAddress, ETHER_UINT_800);
    // userB lptoken balance should increase by withdraw amount
    expect(await lpToken.balanceOf(userBAddress)).to.equal(
      userBLptokenBefore.add(ETHER_UINT_800)
    );
  ...
  it("Should emergency withdraw for userA", async function () {
    ...
    await bonusRewards
      .connect(userAAccount)
      .emergencyWithdraw([lpTokenAddress]);
  ...
  it("Should NOT collectDust on bonusToken if active", async function () {
    await expectRevert(
      bonusRewards.collectDust(bonusToken.address, lpToken.address, 0),
  ...
  it("Should add 2nd Bonus, update Bonus weekly rewards and startTime, and claim", async function () {
    ...
    await bonusRewards
      .connect(userAAccount)
      .claimRewardsForPools([bonusToken.address, lpToken.address]);
    await bonusRewards
      .connect(userBAccount)
      .claimRewardsForPools([lpToken.address]);
    ...
    expect(balB).to.lt(
      WEEKLY_REWARDS_DOUBLE.add(WEEKLY_REWARDS).mul(11).div(70)
    );
  });
  ...
  it("Should only collectDust on bonus tokens on eligible lpTokens", async function () {
    ...
    await bonusRewards.addPoolsAndAllowBonus(
      [lpToken.address, lpToken2.address],
      [bonusToken.address],
      [partnerAddress]
    );
    const latest = await time.latest();
    ...
    expect(await bonusToken.balanceOf(bonusRewards.address)).to.equal(
      WEEKLY_REWARDS_DOUBLE.add(WEEKLY_REWARDS)
    );
    const [[[, , , weeklyRewards, , remBonus]]] = await bonusRewards.getPool(
      lpToken2.address
    );
